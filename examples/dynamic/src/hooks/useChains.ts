import {
  ChainType,
  createClient,
  type ExtendedChain,
  getChains,
} from '@lifi/sdk'
import { convertExtendedChain } from '@lifi/widget-provider-ethereum'
import { useQuery } from '@tanstack/react-query'
import { useMemo } from 'react'

const client = createClient({ integrator: 'vite-example' })

const chainTypes = [ChainType.EVM, ChainType.SVM, ChainType.UTXO, ChainType.MVM]

export function useChains() {
  const { data, isLoading } = useQuery({
    queryKey: ['lifi-chains'],
    queryFn: async () => {
      const chains = await getChains(client, { chainTypes })
      client.setChains(chains)
      return chains
    },
    refetchInterval: 300_000,
    staleTime: 300_000,
  })

  const evmExtendedChains = useMemo(
    () => data?.filter((c: ExtendedChain) => c.chainType === 'EVM'),
    [data]
  )

  const evmChains = useMemo(
    () => evmExtendedChains?.map(convertExtendedChain),
    [evmExtendedChains]
  )

  return {
    chains: data,
    evmExtendedChains,
    evmChains,
    isLoading,
  }
}
