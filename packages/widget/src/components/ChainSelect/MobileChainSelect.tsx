...
import {
  MobileChainCard,
  MobileChainContainer,
} from './MobileChainSelect.style.js';
import { useChainSelect } from './useChainSelect.js';

const mobileMaxChainsToShow = 6;
const mobileMaxChainsToOrder = 5;

export const MobileChainSelect = ({ formType }: FormTypeProps) => {
  ...
  const chainsToHide =
    chains?.length === mobileMaxChainsToShow
      ? 0
      : (chains?.length ?? 0) - mobileMaxChainsToOrder;
  ...
  return (
    <MobileChainContainer>
      {isLoading
        ? Array.from({ length: mobileMaxChainsToOrder }).map((_, index) => (
            <Skeleton
              key={index}
              variant="rectangular"
              width={52}
        ...
        : chainsToShow
            ?.slice(0, mobileMaxChainsToOrder)
            .map((chain: EVMChain) => (
      ...
      {chainsToHide > 0 ? (
        <MobileChainCard component="button" onClick={showAllChains}>
          <Box
            sx={{
              width: 40,
              height: 40,
