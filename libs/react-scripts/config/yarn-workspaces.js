...
const init = (paths, webpackEnv) => {
	...
	const config = {
		...
		imports: [],
		packageEntry: 'module',
		development: webpackEnv === 'development',
	...
	// For tree shaking in development
	// if (config.paths.length > 0) {
	// 	const exceptions = ['icons'];
	// 	config.imports = config.paths.reduce((directories, path) => {
	// 		return directories.concat(fs.readdirSync(path, { withFileTypes: true })
